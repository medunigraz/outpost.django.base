...
class ForeignDataWrapperKey(models.ForeignKey):
    def __init__(
        self, to, fields=None, exclude=None, algorithm="sha256", *args, **kwargs
    ):
        self.fields = fields
    ...
    def contribute_to_class(self, cls, name):
        if not cls._meta.abstract:
            hash_field = models.BinaryField(
                editable=False, blank=self.blank, null=self.null
            )
        ...
        setattr(
            ...
            ForeignDataWrapperDescriptor(
                self, name, self.algorithm, _filter_fields(self.fields, self.exclude)
            ),
        )
    ...
    def pre_save(self, model_instance, add):
        ...
        if getattr(model_instance, _hash_field_name(self.name)) != hashsum:
            foreign_data_changed.send(
                sender=self.__class__, instance=model_instance, field=self.name
            )
